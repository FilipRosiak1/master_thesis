...
% -- poprawka F
by itself establish an advantage over the Framsticks EA baseline without the paired statistics defined earlier. The completed LSO runs and the standard baseline runs used the same four-file simulation chain, so the remaining caution concerns retained-policy semantics and checkpoint dependence rather than a simulation-chain mismatch.
...
Finally, the evidence supports only limited generalization. The final LSO benchmark used five checkpoints, three optimizers, one \fone{} dataset, one fitness criterion, 
% -- poprawka F
% one LSO simulation configuration, 
the standard four-file simulation chain, 
and 150 structured starting genotypes. Results may change for another genotype language, simulator configuration, fitness objective, training distribution, or latent dimensionality. 
% Moreover, neither completed baseline is a same-condition comparator for the LSO scores: the standard baseline adds deterministic, sample-period, and body-only simulation files to \texttt{eval-allcriteria.sim}, while the mini baseline uses the corresponding four-file chain beginning with \texttt{eval-allcriteria-mini.sim}. Reusing the same starting-genotype manifest does not remove this simulation-configuration confound. 
The standard baseline shares this chain and the starting-genotype manifest, so it is the same-condition baseline for LSO. The mini baseline remains a separate condition because it uses the corresponding four-file chain beginning with \texttt{eval-allcriteria-mini.sim}.
% ---
Broader conclusions require repeated training seeds, independently selected checkpoints, additional tasks, and evaluation conditions that remain identical across the methods being compared.
