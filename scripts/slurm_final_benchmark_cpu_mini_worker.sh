...
RUN_DIR="${1:?Usage: slurm_final_benchmark_cpu_mini_worker.sh RUN_DIR [JOB_PLAN] [JOB_INDEX]}"
...
python scripts/run_final_benchmark_job.py \
  ...
  --framsticks-sim "src/framsticks/framspy/eval-allcriteria-mini.sim;src/framsticks/framspy/deterministic.sim;src/framsticks/framspy/sample-period-2.sim;src/framsticks/framspy/only-body.sim" \
  --generations 300 \
  --population-size 100 \
  --offspring-size 100 \
  --elite-size 10 \
